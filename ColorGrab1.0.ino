  ...
  setColor(dim(colors[color],deadBrightness));
    ...
    FOREACH_FACE(f){
      if(!isValueReceivedOnFaceExpired(f)){
        if (getColor(getLastValueReceivedOnFace(f))==color){
          if(spinTimer.isExpired()){
            displayFace++;
            spinTimer.set(LAP_DURATION);
           }
          setColor(dim(colors[color],80));
          setColorOnFace(colors[color],displayFace%6);
        }
      }else{
